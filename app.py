...
# This is a free Hugging Face Space endpoint (no token needed for public use)
...
def generate():
    try:
        ...
        # Hugging Face Spaces usually return something like:
        # {"data": ["https://.../output.png"]}
        if "data" in result and len(result["data"]) > 0:
            return jsonify({"result": result["data"][0]})
        else:
            return jsonify({"error": "Invalid response from free model", "raw": result}), 500
